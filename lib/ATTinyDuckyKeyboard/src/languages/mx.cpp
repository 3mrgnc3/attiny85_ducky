// Mexican Spanish (Latin America) keyboard layout for ATTiny85


// Based on USB Rubber Ducky mx.json mapping
// ASCII characters 32-126 mapped to USB HID keycodes

#include "languages.h"

const uint16_t lang_mx[] PROGMEM = {
    KEY_SPACE,                                      // 32 ' '
    ...
    KEY_1,                                          // 49 1
    KEY_2,                                          // 50 2
    (KEY_MOD_LSHIFT << 8) | KEY_3,                         // 51 3
    (KEY_MOD_LSHIFT << 8) | KEY_4,                         // 52 4
    (KEY_MOD_LSHIFT << 8) | KEY_5,                         // 53 5
    (KEY_MOD_LSHIFT << 8) | KEY_6,                         // 54 6
    (KEY_MOD_LSHIFT << 8) | KEY_7,                         // 55 7
    (KEY_MOD_LSHIFT << 8) | KEY_8,                         // 56 8
    ...
    KEY_2 | (KEY_MOD_RALT << 8),                    // 64 @ (AltGr+Q)
    ...
    KEY_LEFTBRACE | (KEY_MOD_RALT << 8),            // 91 [ (AltGr+´)
    KEY_BACKSLASH,                                  // 92 \ (key left of Z)
    ...
    KEY_Y,                                          // 121 y
    KEY_Z,                                          // 122 z
    KEY_APOSTROPHE | (KEY_MOD_RALT << 8),           // 123 { (AltGr+{)
    KEY_BACKSLASH | (KEY_MOD_RALT << 8),            // 124 | (AltGr+|)
    KEY_EQUAL | (KEY_MOD_RALT << 8),                // 125 } (AltGr+})
    KEY_RIGHTBRACE | (KEY_MOD_RALT << 8)            // 126 ~ (AltGr+~)
};