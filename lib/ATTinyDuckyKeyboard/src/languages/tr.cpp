// Turkish Q keyboard layout for ATTiny85


// Based on USB Rubber Ducky tr.json mapping
// ASCII characters 32-126 mapped to USB HID keycodes

#include "languages.h"

const uint16_t lang_tr[] PROGMEM = {
    KEY_SPACE,                                      // 32 ' '
    ...
    (KEY_MOD_LSHIFT << 8) | KEY_APOSTROPHE,                // 59 ;
    KEY_COMMA,                                      // 60 <
    (KEY_MOD_LSHIFT << 8) | KEY_0,                         // 61 =
    (KEY_MOD_LSHIFT << 8) | KEY_COMMA,                     // 62 >
    (KEY_MOD_LSHIFT << 8) | KEY_SLASH,                     // 63 ?
    (KEY_MOD_RALT << 8) | KEY_Q,                    // 64 @ (AltGr+Q)
    ...
    (KEY_MOD_RALT << 8) | KEY_8,                    // 91 [ (AltGr+8)
    (KEY_MOD_RALT << 8) | KEY_MINUS,                // 92 \ (AltGr+-)
    (KEY_MOD_RALT << 8) | KEY_9,                    // 93 ] (AltGr+9)
    KEY_3 | (KEY_MOD_RALT << 8),                    // 94 ^ (AltGr+3)
    (KEY_MOD_LSHIFT << 8) | KEY_MINUS,                     // 95 _
    (KEY_MOD_LSHIFT << 8) | KEY_BACKSLASH,                 // 96 `
    KEY_A,                                          // 97 a
    KEY_B,                                          // 98 b
    KEY_C,                                          // 99 c
    KEY_D,                                          // 100 d
    ...
    KEY_Y,                                          // 121 y
    KEY_Z,                                          // 122 z
    (KEY_MOD_RALT << 8) | KEY_7,                    // 123 { (AltGr+7)
    (KEY_MOD_RALT << 8) | KEY_BACKSLASH,            // 124 | (AltGr+*)
    (KEY_MOD_RALT << 8) | KEY_0,                    // 125 } (AltGr+0)
    (KEY_MOD_RALT << 8) | KEY_RIGHTBRACE            // 126 ~ (AltGr+ü)
};