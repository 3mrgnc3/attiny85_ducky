// Catalan QWERTY keyboard layout for ATTiny85


// Based on USB Rubber Ducky ca.json mapping
// ASCII characters 32-126 mapped to USB HID keycodes

#include "languages.h"

const uint16_t lang_ca[] PROGMEM = {
    KEY_SPACE,                                      // 32 ' '
    ...
    (KEY_MOD_RALT << 8) | KEY_APOSTROPHE,           // 92 \ (AltGr+')
    (KEY_MOD_RALT << 8) | KEY_EQUAL,                // 93 ] (AltGr++)
    (KEY_MOD_RALT << 8) | (KEY_MOD_LSHIFT << 8) | KEY_LEFTBRACE,  // 94 ^ (AltGr+Shift+`)
    ...
    KEY_Y,                                          // 121 y
    KEY_Z,                                          // 122 z
    (KEY_MOD_RALT << 8) | (KEY_MOD_LSHIFT << 8) | KEY_APOSTROPHE, // 123 { (AltGr+Shift+')
    (KEY_MOD_RALT << 8) | KEY_1,                    // 124 | (AltGr+1)
    (KEY_MOD_RALT << 8) | (KEY_MOD_LSHIFT << 8) | KEY_EQUAL,      // 125 } (AltGr+Shift++)
    (KEY_MOD_RALT << 8) | KEY_4                     // 126 ~ (AltGr+4)
};